// src/components/Dashboard/Authors/Modals/CreateAuthorModal.tsx
...
const CreateAuthorModal = ({ isOpen, onClose, onSubmit }: CreateAuthorModalProps) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (name === "genres") {
      setFormData({
        ...formData,
        genres: value.split(",").map((genre) => genre.trim()).filter(Boolean),
      });
    } else {
      setFormData({
        ...formData,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({
        submit: "Failed to create author. Please try again.",
      });
    } finally {
      setIsLoading(false);
    }
